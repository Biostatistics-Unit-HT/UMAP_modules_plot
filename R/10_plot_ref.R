...
#
# `label_col` (optional) overrides the column used to anchor the centroid
# labels. Defaults to `join_col` so behaviour is unchanged when the
# orchestrator does not pass anything.
#
# Example:
#   plot_ref(merged, "GH Reference", "celltype_2", 0.25, TRUE)
#   # -> ggplot of the full UMAP in palette colours, labelled by celltype_2.
#   plot_ref(merged, "GH Reference", "celltype_2", 0.25, TRUE,
#            label_col = "celltype_3")
#   # -> coloured by ct2 palette, labels drawn at ct3 granularity.
plot_ref <- function(df, title_text, join_col, pt_size, use_raster,
  ...
  p + scale_color_identity() +
    geom_text_repel(data = centroids, aes(label = .data[[group_col]]),
